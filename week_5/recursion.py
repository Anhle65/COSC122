...
def quick_power(x, n):
    ...
    # ---start student section---
    if n < 0 or n % 1 != 0:
        raise ValueError("n must be interger and non-negative")
    else:
        if n == 0:
            return 1
        elif n % 2 == 0:
            n = n//2
            return(quick_power(x, n)*quick_power(x, n))
        else:
            n -= 1
            return(x * quick_power(x, n))
    # ===end student section===
...
def linked_list_length(list_node):
    ...
    if list_node == None:
        ...
        return linked_list_length(list_node) + 1
    # ===end student section===
...
def linked_list_reverse_print(list_node):
    ...
    if list_node == None:
        ...
        value = list_node.item
        list_node = list_node.next_node
        linked_list_reverse_print(list_node)
        print(value)
...
def is_in_linked_list(list_node, item):
    ...
    if list_node == None:
        ...
        if list_node.item == item:
            ...
            list_node = list_node.next_node
            return is_in_linked_list(list_node, item)
    # ===end student section===
...
def recursive_string_print(s):
    ...
    if length > 0:
        ...
        recursive_string_print(s[length - length + 1:])
    # ===end student section===
...
def recursive_reverse_string_print(s):
    ...
    if length > 0:
        print(s[length - 1])
        recursive_reverse_string_print(s[0:length-1])
    # ===end student section===


def recursive_list_print(alist, start_index=0):
    """prints out the elements in the list one per line, 
    from the item at start_index through to the last item in the list. 
    If the start_index is >= the length of the list then nothing should be printed - this is the base case.
    must NOT use list slicing"""
...
def squares(data):
    ...
    if not data:
        return []
    first_element = data[0]*data[0]
    rest_elements = squares(data[1:])
...
def num_rushes(slope_height, rush_height_gain, back_sliding, start_height=0):
    ...
    if start_height >= slope_height:
        ...
        current = start_height + rush_height_gain
        if current < slope_height:
            current = start_height + rush_height_gain - back_sliding
        return num_rushes(slope_height, rush_height_gain, back_sliding, current) + 1
