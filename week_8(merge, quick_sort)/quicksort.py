...
print(len(read_data("data/list4.txt")))
...
def quicksort_range_helper(values, left, right, start, end, style):
    ...
    # ===end student section===
print(quicksort(read_data("data/list4.txt"))[375])
...
def pivot_index_mo3(values, left, right):
    ...
    middle = (left + right) // 2
    find_median = [values[left], values[middle], values[right]]
    # ---start student section---
    if len(values) <= 2:
        return left
    else:
        if values[left] == values[middle] == values[right]:
            return middle
        for i in range(len(find_median)):
            current = find_median[i]
            position = i
            while position > 0 and find_median[position-1] > current:
                find_median[position] = find_median[position-1]
                position -= 1
            find_median[position] = current
        median = find_median[1]
        index = values.index(median)
        return index
    # ===end student section===
# values = read_data("data/list5.txt")
